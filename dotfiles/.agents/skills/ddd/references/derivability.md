...
On convergence the merged output becomes `## Tasks` and the acceptance executables. The implementer later *inherits* these and never authors the criteria it is graded by — that decorrelation is the whole point. Derivers are read-only on the shared working tree (SKILL.md §2).
